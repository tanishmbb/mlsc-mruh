...
export default function GalleryPage() {
  ...
  const openImageViewer = (src: string, title: string, images: string[], currentIndex: number) => {
    ...
    <div className="min-h-screen bg-card">
      ...
      <main className="pt-16">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              ...
              <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
                Explore the memorable moments from our events, workshops, and community activities that showcase the
                vibrant spirit of MLSC MRUH.
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <section className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Event Galleries</h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            ...
            <div className="space-y-16">
              {eventGalleries.map((gallery, index) => (
                <EventGallery key={gallery.id} gallery={gallery} index={index} onImageClick={openImageViewer} />
              ))}
            </div>
          </div>
        </section>
      ...
      {selectedImage && (
        <ImageViewer
          ...
          onNavigate={(newIndex) => setSelectedImage((prev) => (prev ? { ...prev, currentIndex: newIndex } : null))}
        />
      )}
    </div>
  )
